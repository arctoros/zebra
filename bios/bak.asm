...
SIO_A_DATA = $00							; Port A data register
SIO_B_DATA = $01							; Port B data register
SIO_A_CMD	= $02								; Port A command register
SIO_B_CMD	= $03								; Port B command register
...
parse:	cp $0A             ; Enter?
        jp z, jump			   ; yes, go finish

        cp '0'     			  ; Convert ASCII -> hex nibble
        jr c, monitor
        cp '9'+ 1
        jr c, digit
        cp 'a'
...
hex:		ld hl, (JMP_ADDRESS); HL = current accumulated value
     		add hl, hl						; Shift left 4 and add new nibble
        add hl, hl
        add hl, hl
        add hl, hl
        ld e, a
...
jump:		ld hl, (JMP_ADDRESS)
				call printChar
      	ld a, h
      	cp $80
      	jr c, resetAddress    ; below 0x8000
      	jp (hl)               ; jump to parsed address
;---

;+-- Subroutines
printChar:
				push bc
				out (SIO_B_DATA), a			; Print A to SIO
				ld b, 40
...
bitLoop:
				...
				jp z, null						; Only print if not null
...
;+-- Includes
				...
				;.include "snail.asm"
				;org $D000
				;.include "psg/bios.asm"
				;org $AFF4
				;include "misc/helloworld.asm"
				;org $F700
				;.include "pio.asm"
;---
