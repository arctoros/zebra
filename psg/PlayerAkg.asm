...
;       To use a ROM player (no automodification, use of a small buffer to put in RAM):
	      PLY_AKG_ROM = 1
  	    PLY_AKG_ROM_Buffer = #7000; (or wherever).
...
PLY_AKG_Channel{channelNumber}_SetInstrumentStep:
        ...
        if {channelNumber} != 3
        	rr d            ;On ZX Spectrum, just rotate right.
        else
         	;Gets the R7.
          ld a,d
        endif
...
; PSG access.
;   - A holds R7 on entry
;   - HL used as storage for reg pairs (R0/1, R2/3, etc.)
;   - Other regs as per AKG player’s calling convention
; -----------------------------------------------------------------------------------
...
PLY_AKG_SendPSGRegisters:
        ...
        ld b,a          ;Preserves R7.
        ld a,7
        out (PSG_ADDR),a     ;Register.
        ld a,b
        out (PSG_WRITE),a     ;Value.
...
dkws (void):    ;Disark macro.
        ...
        ; PSG running at 1843200 Hz
        dw 7045, 6650, 6277, 5924, 5592, 5278, 4982, 4702, 4438, 4189, 3954, 3732   ; Octave 0
        dw 3522, 3325, 3138, 2962, 2796, 2639, 2491, 2351, 2219, 2095, 1977, 1866   ; Octave 1
        dw 1762, 1662, 1569, 1481, 1398, 1319, 1245, 1176, 1110, 1047, 988, 933     ; Octave 2
        dw 881, 831, 785, 741, 699, 660, 623, 588, 555, 524, 494, 467               ; Octave 3
        dw 440, 416, 392, 370, 349, 330, 311, 294, 277, 262, 247, 233               ; Octave 4
        dw 220, 208, 196, 185, 175, 165, 156, 147, 139, 131, 124, 117               ; Octave 5
        dw 110, 104, 98, 93, 87, 82, 78, 73, 69, 65, 62, 58                         ; Octave 6
        dw 55, 52, 49, 46, 44, 41, 39, 37, 35, 33, 31, 29                           ; Octave 7
        dw 28, 26, 25, 23, 22, 21, 19, 18, 17, 16, 15, 15                           ; Octave 8
        dw 14, 13, 12, 12, 11, 10, 10, 9, 9, 8, 8, 7                               ; Octave 9
        dw 7, 7, 6, 6, 6, 5, 5, 5                                                  ; Octave 10
